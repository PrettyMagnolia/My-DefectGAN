...
class DefectGANDataset(data.Dataset):
    ...
    def load_image_class_and_image_path_from_index(self, batch_index: int):
        ...
        normal_image_path = self.normal_image_list[batch_index]
        defect_image_path = self.defect_image_list[batch_index]
        defect_mask_path = self.defect_mask_list[batch_index]
        ...
        defect_mask_tensor = torch.where(defect_mask_tensor != 0, 1, 0)
    ...
    def __len__(self):
        return len(self.defect_image_list)
...
if __name__ == '__main__':
    defect_dataset = DefectGANDataset("./datasets", num_classes=5, device_id='0')
